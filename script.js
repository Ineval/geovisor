    ...
    const levels = {
      ...
      bachillerato: { label: 'Bachillerato', pointRpc: 'get_3_bachillerato_sest25_geojson', base: 'bachillerato', color: '#ff8c00', subjects: ['general', 'mt', 'lyl', 'fis', 'qui', 'his', 'bio', 'ed', 'fil'] }
    };
    ...
    async function callRpcWithAlternatives(names, params = {}) {
      ...
      for (const name of alternatives) {
        try {
          ...
          lastError = error;
          console.warn('No se pudo cargar RPC:', name, error);
        }
      }
      throw lastError;
    }

    function getPointRpcAlternatives(levelKey) {
      const primary = levels[levelKey].pointRpc;
      if (levelKey === 'bachillerato') {
        return [primary, 'get_3_bachillerato_ses25_geojson', 'get_bachillerato_sest25_geojson'];
      }
      return [primary];
    }
    ...
    async function loadGeoportal() {
      try {
        ...
        const levelLoads = await Promise.allSettled(
          Object.keys(levels).map(async l => {
            rawPoints[l] = await callRpcWithAlternatives(getPointRpcAlternatives(l));
          })
        );

        levelLoads.forEach((result, index) => {
          if (result.status === 'rejected') {
            const levelKey = Object.keys(levels)[index];
            errors.push(`${levels[levelKey].label}: ${result.reason.message}`);
            rawPoints[levelKey] = { type: 'FeatureCollection', features: [] };
          }
        });

        try {
          const limiteData = await callRpc('get_limite_provincial_geojson');
          limitLayer = L.geoJSON(limiteData, {
            pane: 'linesPane',
            style: () => ({ pane: 'linesPane', fillColor: 'transparent', color: '#263746', weight: 1.3, opacity: .85, fillOpacity: 0 }),
            onEachFeature: (f, layer) => {
              const p = f.properties || {};
              if (p.PROVINCIA) layer.bindTooltip(p.PROVINCIA, { permanent: true, direction: 'center', className: 'province-label', interactive: false });
            }
          });
        } catch (error) {
          errors.push(`Límites provinciales: ${error.message}`);
          console.warn(error);
        }
        ...
        if (errors.length) {
          showStatus('Conexión parcial. Revisa la consola del navegador para ver las RPC que fallaron.', 'warning');
          console.warn('Errores de carga del geovisor:', errors);
        } else {
          showStatus('');
        }
      } catch (error) {
