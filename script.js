const SUPABASE_URL = "https://apiqfherndzzulzwrryq.supabase.co";
    ...
    function getPointRpcAlternatives(levelKey) {
      if (levelKey === 'bachillerato') {
        return ['get_3_bachillerato_sest25_geojson', 'get_3_bachillerato_ses25_geojson'];
      }
      return [levels[levelKey].pointRpc];
    }

    async function callFirstRpc(names) {
      let lastError = null;
      for (const name of names) {
        try { return await callRpc(name); }
        catch (error) { lastError = error; console.warn('RPC no disponible:', name, error); }
      }
      throw lastError;
    }
    ...
    async function loadGeoportal() {
      try {
        ...
        Object.keys(levels).forEach(l => {
          loads.push(
            callFirstRpc(getPointRpcAlternatives(l))
              .then(data => { rawPoints[l] = data; })
              .catch(error => { console.warn('No se pudo cargar el nivel:', l, error); rawPoints[l] = { type: 'FeatureCollection', features: [] }; })
          );
        });
        ...
        }).catch(error => { console.warn('No se pudo cargar límite provincial:', error); limitLayer = null; }));
